...
#include "raisim_teleop/AtlasStepPlanner2D.h"
#include "raisim_teleop/atlas_raisim_teleop.h"
...
int main(int argc, char **argv)
{
  ...
  ros::init(argc, argv, "raism_teleop"); // Initialize node
  ...
  nh.getParam("/atlas_teleop/num_steps",num_steps);
  nh.getParam("/atlas_teleop/velocity_x",speed_x);
  nh.getParam("/atlas_teleop/velocity_y",speed_y);
  nh.getParam("/atlas_teleop/angular_velocity",speed_w);
  nh.getParam("/atlas_teleop/ros_rate_value",ros_rate_value);
  nh.getParam("/atlas_teleop/robot_name",robot_name);
  ...
  tf2::Quaternion q;
  int previous_support{1}; // 1 for left ---- 0 for right
  ...
  while (ros::ok()){
    ...
    // If keystroke is pressed before the finalization of the cotnrol u ll have a problem with the previous step variable.
    // TODO: create a condition and a feedback mechanism, in order to check if atlas has reached the goal and is ready for new command
    ...
    // Last step of previous command
    if (previous_support == 1){
      ...
      target_step.targetSupport = SUPPORT_LEG_LEFT;
      target_step.target[0] = RLeg_odom.pose.pose.position.x;
      target_step.target[1] = RLeg_odom.pose.pose.position.y;
      target_step.target[2] = Quat2Euler(RLeg_odom.pose.pose.orientation.x,RLeg_odom.pose.pose.orientation.y,RLeg_odom.pose.pose.orientation.z,RLeg_odom.pose.pose.orientation.w);
    }
