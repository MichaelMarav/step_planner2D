...
// TODO: Rewrite the whole method because it does't take into account the rotation of the previous foot
WalkInstruction StepPlanner2D::planStep2D(Eigen::Vector3d v, WalkInstruction si) // v -> velocity
{
	...
	WalkInstruction ci;
        ...
        if(v(0)==v_(0) && v(1)==v_(1) && v(2)==v_(2) && v(0)==0 && v(1)==0 && v(2)==0)
        {
            cmd = STAND;
        ...
        ci.target(2) =  targetTheta; // mixalis comment maybe  + targetXY(2)
